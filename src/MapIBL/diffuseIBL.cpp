...
int main( int argc, char **argv )
{
    ...
    // Save irradiance map to file
    // -----------------------------------------------------------------------------------------
    const size_t width = output_w;
    size_t size = 4*width*width*sizeof(GL_UNSIGNED_BYTE);
    uint8_t *data = new uint8_t[size];

    idk::gl::activeTexture(GL_TEXTURE_CUBE_MAP);
    idk::gl::bindTexture(GL_TEXTURE_CUBE_MAP, framebuffer.cubemap_attachment);
    ...
    for (int i=0; i<6; i++)
    {
        ...
        std::string filepath = output_dir + faces[i];
        IMG_SavePNG(img, filepath.c_str());
